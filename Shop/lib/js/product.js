...
function deleteOrderName(name){
    ...
    //load list with the one product deleted
    if(deleted)showDeletedProductsTable();
    //only save when clicked on save
    //if one item deleted before load section with object from itemsfile;
}
...
function showProductTable(){
        $("#products").empty();
        var obj = getJSON();
        //if(deleted) obj = itemsFile; 
        var nr = 0;
        for(i=0; i<obj.drinks.length; i++){
                var product = obj.drinks[i].name;
                var size = obj.drinks[i].size;
            nr++;
            $("#products").append("<div class='prodItem'><div class='tc'>"+nr+"</div><div class='tcp'>"+product+"</div><div class='tc'>"+size+"</div><div class='rem' onclick='deleteOrderName(\""+product+"\");'>Löschen</div></div></div>");
        }
...
//generates product list from variable and appends it to products.php
function showDeletedProductsTable(){
    $("#products").empty();
    var obj = itemsFile;
    console.log(obj)
    var nr = 0;
